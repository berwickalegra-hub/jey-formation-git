...
**Plans**: 7 plans
  - [x] 03-01-scaffold-migration-helpers-PLAN.md — Wave 0: Prisma migration (User.status + Order.idempotencyKey), paginate + per-userId rate-limit helpers, 10 RED test scaffolds + admin fixtures, [BLOCKING] schema push
  - [x] 03-02-admin-reads-users-orders-withdrawals-PLAN.md — Wave 1: GET /api/admin/users (list+detail), GET /api/admin/orders, GET /api/admin/withdrawals (LIST only)
  - [x] 03-03-admin-audit-log-me-and-rate-limit-PLAN.md — Wave 1: GET /api/admin/audit-log, GET /api/admin/me (capability list)
  - [x] 03-04-admin-visibility-outbox-email-rate-limits-PLAN.md — Wave 1: GET /api/admin/outbox (kind, not type), GET /api/admin/email-queue (bodyPreview <=200), GET /api/admin/rate-limits (7 buckets, 1000-key cap)
  - [x] 03-05-orders-create-with-circuit-breaker-PLAN.md — Wave 1: POST /api/orders with Idempotency-Key + lazy Bictorys provider + CircuitBreaker (PAY-01)
  - [x] 03-06-admin-mutations-role-status-cancel-PLAN.md — Wave 2: PATCH role (last-SUPERADMIN guard), PATCH status (ADMIN suspend/SUPERADMIN restore), POST withdrawal cancel (SUPERADMIN)
  - [x] 03-07-protected-edits-and-make-superadmin-PLAN.md — Wave 2: PROTECTED-FILE edits to login + refresh (SUSPENDED check) + make-superadmin CLI script (autonomous: false)
**UI hint**: no
...
**Plans**: 4 plans
  - [x] 06-01-tests-scripts-and-docker-uat-PLAN.md — Wave 0: 7 TEST-02 lib tests + smoke-auth.ts + seed-dev refactor + 2 doc tripwires + Docker UAT
  - [x] 06-02-claude-md-cleanup-PLAN.md — Wave 1: CLAUDE.md targeted edits (3 stale forward-refs + Phase 5 surface appendix)
  - [x] 06-03-readme-rewrite-and-roadmap-fix-PLAN.md — Wave 1: README.md full rewrite (7-section outline) + ROADMAP success-criterion #4 docker flag fix
  - [x] 06-04-status-md-refresh-PLAN.md — Wave 1: STATUS.md ✅ DONE Phase 2-5 entries + 🔨 TODO reduction to Phase 6/7
