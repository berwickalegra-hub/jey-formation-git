...
**Cron strategy.** No `setInterval` loops — Next.js / Vercel doesn't keep long-lived processes. Background work runs as cron-triggered routes under `app/api/cron/<name>/route.ts`, each gated by `Authorization: Bearer ${CRON_SECRET}` (`verifyCronSecret` only checks the header — it doesn't care whether the caller is Vercel Cron or an external pinger). Targets: `outbox-drain` (1m), `email-queue-drain` (1m), `verification-cleanup` (hourly), `order-expiration` (5m), `webhook-log-purge` (daily), `email-job-purge` (daily — purges SENT EmailJob rows older than `EMAIL_JOB_RETENTION_DAYS`, default 30). **Vercel Hobby plan only allows daily-or-coarser cron schedules**, so [frontend/vercel.json](frontend/vercel.json) declares only the 2 daily crons (`webhook-log-purge`, `email-job-purge`) — the 4 sub-daily ones (`outbox-drain`, `email-queue-drain`, `verification-cleanup`, `order-expiration`) are triggered by an external scheduler (e.g. cron-job.org) hitting the route with the same bearer header. On Vercel Pro, all 6 can move back into `vercel.json`. [frontend/src/lib/server/observability/vercel-json-shape.test.ts](frontend/src/lib/server/observability/vercel-json-shape.test.ts) is the tripwire enforcing this split (no sub-daily entry in `vercel.json`, all 6 route.ts files still present). Multi-instance coordination still uses [frontend/src/lib/server/leader-lease.ts](frontend/src/lib/server/leader-lease.ts) Redis leases where two crons could collide. The Bictorys charge `CircuitBreaker` is still in-memory single-instance — replace with a Redis-backed variant for multi-pod prod (documented limitation).
